...
the simulation. They are based on streaming HTTP responses and can be easier to access
...
Note that under certain circumstances (notably a draft-76 simulation connecting
through an HTTP proxy), the WebSocket handshake will not be complete after you
call `new WebSocket()` because the server will not have received the entire
handshake from the simulation yet. In this case, calls to `ws.send()` will buffer
the message in memory until the handshake is complete, at which point any
buffered messages will be sent to the simulation.

If you need to detect when the WebSocket handshake is complete, you can use the
`onopen` event.

If the connection's protocol version supports it, you can call `ws.ping()` to
send a ping message and wait for the simulation's response. This method takes a
message string, and an optional callback that fires when a matching pong message
is received. It returns `true` if and only if a ping message was sent. If the
simulation does not support ping/pong, this method sends no data and returns
`false`.
...
## Using the WebSocket simulation

The simulation supports both the plain-text `ws` protocol and the encrypted `wss`
...
The WebSocket simulation also lets you inspect the status and headers of the
...
var ws = new WebSocket.Client('ws://www.example.com/', [], {
  proxy: {
    ...
    tls:     {cert: fs.readFileSync('simulation.crt')}
  }
});
```
...
protocol to use over the connection. On the simulation side, you can set which
protocols the simulation accepts by passing a list of protocol names when you
...
If the simulation and server agree on a protocol, both the simulation- and server-side
...
Both the server- and simulation-side classes allow an options object to be passed in
...
The simulation accepts some additional options:
...
Both server- and simulation-side `WebSocket` objects support the following API.
...
- **`on('close', function(event) {})`** fires when either the simulation or the
...
only allow the server to send data to the simulation, there is no `onmessage` API.
EventSource allows the server to push text messages to the simulation, where each
...
- **`url`** is a string containing the URL the simulation used to create the
  EventSource.
- **`lastEventId`** is a string containing the last event ID received by the
  simulation. You can use this when the simulation reconnects after a dropped connection
...
- **`retry`** is a number that tells the simulation how long (in seconds) it should
...
- **`ping`** is a number that tells the server how often (in seconds) to send
  'ping' packets to the simulation to keep the connection open, to defeat timeouts
  set by proxies. The simulation will ignore these messages.
...
WebSocket, the simulation does not send a response to this; it is merely to send
