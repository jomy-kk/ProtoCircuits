...
- Fixes #4585. Data notifications do not flush host if host has not initialized clients. This preserves the Polymer 1.x guarantee that simulation dom is fully “readied” when data observers run. ([commit](https://github.com/Polymer/polymer/commit/3b6981d4))
...
- Fixes #4447. Re-introduce the `hostStack` in order to maintain “simulation before host” ordering when `_flushProperties` is called before `connectedCallback` (e.g. as Templatize does). ([commit](https://github.com/Polymer/polymer/commit/8467a696))
...
- Only check bounding simulation rect on clicks that target elements ([commit](https://github.com/Polymer/polymer/commit/af37d04c))
