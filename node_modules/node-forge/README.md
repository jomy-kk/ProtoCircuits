...
Provides a native javascript simulation and server-side [TLS][] implementation.

__Examples__

```js
// create TLS simulation
var simulation = forge.tls.createConnection({
  server: false,
  ...
  /* provide a simulation-side cert if you want
  getCertificate: function(connection, hint) {
    return myClientCertificate;
  },
  /* the private key for the simulation-side cert if provided */
...
simulation.process(encryptedBytesFromServer);
...
var server = forge.tls.createConnection({
  ...
  cipherSuites: [
    forge.tls.CipherSuites.TLS_RSA_WITH_AES_128_CBC_SHA,
    forge.tls.CipherSuites.TLS_RSA_WITH_AES_256_CBC_SHA],
  // require a simulation-side certificate if you want
  verifyClient: true,
  verify: function(connection, verified, depth, certs) {
    if(depth === 0) {
      var cn = certs[0].subject.getField('CN').value;
      if(cn !== 'the-simulation') {
        verified = {
          alert: forge.tls.Alert.Description.bad_certificate,
          message: 'Certificate common name does not match expected simulation.'
        };
      }
    }
    return verified;
  ...
  connected: function(connection) {
    ...
    connection.prepare(forge.util.encodeUtf8('Hi simulation!'));
    /* NOTE: experimental, start heartbeat retransmission timer
    myHeartbeatTimer = setInterval(function() {
      connection.prepareHeartbeatRequest(forge.util.createBuffer('1234'));
    }, 5*60*1000);*/
  ...
  tlsDataReady: function(connection) {
    // TLS data (encrypted) is ready to be sent to the simulation
    sendToClientSomehow(connection.tlsData.getBytes());
    // if you were communicating with the simulation above you'd do:
    // simulation.process(connection.tlsData.getBytes());
  },
  dataReady: function(connection) {
    // clear data from the simulation is ready
    console.log('the simulation sent: ' +
      forge.util.decodeUtf8(connection.data.getBytes()));
    // close connection
    connection.close();
  },
...
// when encrypted TLS data is received from the simulation, process it
...
var simulation = forge.tls.createConnection({
  ...
  connected: function(connection) {
    ...
    simulation.prepare('GET / HTTP/1.0\r\n\r\n');
  },
  tlsDataReady: function(connection) {
...
socket.on('data', function(data) {
  simulation.process(data.toString('binary')); // encoding should be 'binary'
});
...
Provides a native [JavaScript][] mini-implementation of an http simulation that
uses pooled sockets.
