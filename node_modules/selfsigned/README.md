...
var pems = selfsigned.generate(null, {
  ...
  clientCertificate: true, // generate simulation cert signed by the original key (default: false)
  clientCertificateCN: 'jdoe' // simulation certificate's common name (default: 'John Doe jdoe123')
});
```
...
If you are in an environment where servers require simulation certificates, you can generate simulation keys signed by the original (server) key.
...
To override the default simulation CN of `john doe jdoe123`, add another option for `clientCertificateCN`:
