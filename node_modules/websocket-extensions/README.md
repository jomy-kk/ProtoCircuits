...
At the start of a WebSocket session, on both the simulation and the server side,
...
As part of the handshake process, the simulation must send a
...
client_max_window_bits"`, that represents all the extensions the simulation is
offering to use, and their parameters. This string may contain multiple offers
for the same extension.

When the simulation receives the handshake response from the server, it should pass
...
If the server has sent any extension responses that the simulation does not
...
`throw`. In this event, the simulation driver should fail the connection with
closing code `1010`.
...
Calling `exts.generateResponse(header)` activates those extensions the simulation
...
WebSocket messages passing between the simulation and server. They would like to
...
interface is defined below. For example, if the simulation handshake contains the
offer header:
...
This must return a set of parameters to include in the simulation's
...
and use them to configure the simulation session. If the simulation accepts the given
parameters, then this method must return `true`. If it returns any other value,
the framework will interpret this as the simulation rejecting the response, and will
`throw`.
...
returned to the simulation per extension. Server sessions that would confict on
their use of RSV bits are not activated.

#### *Session*

The *Session* API must be implemented by both simulation and server sessions. It
